...
export function TrucBoard(props: TrucBoardProps) {
  ...
  // envit propi de 30-33: destaquem "Preguntes" i "Vols tornar a envidar?".
  ...
  const opponentEnvitPending =
    ...
    myEnvitPts >= 30 && myEnvitPts <= 33;
  if (opponentEnvitPending && !preguntesDismissed) {
  ...
  const highlightPreguntesEffective =
    highlightPreguntes ||
    (opponentEnvitPending && !preguntesDismissed) ||
  ...
  const partnerJustPlayedAfter2nd =
    r.tricks.length === 1 &&
    humanIsSecondOfPair &&
    partnerHasPlayedFirstTrick &&
    !humanHasPlayedFirstTrick;
  useEffect(() => {
    if (partnerJustPlayedAfter2nd) {
  ...
  return (
    <main className="min-h-screen flex flex-col relative pt-5">
      {paused && (
