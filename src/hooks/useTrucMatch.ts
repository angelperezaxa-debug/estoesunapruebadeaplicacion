  ...
  useEffect(() => {
    ...
    if (
      ...
      if (wouldAccept) {
        ...
        // Davant d'una falta-envit, els bots no pregunten "Vols tornar a
        // envidar?" (no es pot pujar més). Pregunten "Quant envit tens?" i
        // el company respon amb el seu valor real ("Tinc {n}").
        const isFaltaResponse =
          r.envitState.kind === "pending" && r.envitState.level === "falta";
        const consultQuestion: ChatPhraseId = isFaltaResponse
          ? "quant-envit"
          : "vols-tornar-envidar";
        scheduleConsultTimer(() => {
    ...
    if (
      ...
      if (askPartner && sayRef.current) {
        scheduleConsultTimer(() => {
          ...
          if (partnerIsBot) {
            const answer = partnerAnswerFor(match, partner, "vols-envide", bluffRateRef.current);
